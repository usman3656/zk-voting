...
async function main() {
  ...
  console.log(`🚀 AUTOMATED TESTNET SETUP: Deploying to ${networkName.toUpperCase()}`);
  console.log('='.repeat(70));
  ...
  if (!isTestnet) {
    console.error(`\n❌ Error: ${networkName} is not a supported testnet.`);
    console.log(`   Supported testnets: ${testnets.join(', ')}`);
    console.log(`   For local development, use: npm run setup`);
    process.exit(1);
  }
  
  console.log(`\n⚠️  Deploying to ${networkName} testnet`);
  console.log('   Make sure you have:');
  console.log(`   1. ${networkName.toUpperCase()}_RPC_URL set in your root .env file`);
  console.log(`   2. ${networkName.toUpperCase()}_PRIVATE_KEY set in your root .env file`);
  ...
  try {
    ...
    console.log(`   2. Connect MetaMask to ${networkName} network`);
    console.log(`   3. Use the contract at: ${contractAddress}`);
